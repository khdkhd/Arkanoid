...
const Controller = state => {
	...
	function create_cell({ x, y }) {
		...
		return Object.assign({
			note: Note({
				note: notes[_y],
				octave: state.octave,
				duration: 'QUARTER' // TODO READ DURATION
			}),
			pos: _x,
			y: _y
		},
	...
	function update_position(value) {
		...
		state.cursor.onRender(state.screen)
	}
	...
	return {
		...
		updatePattern(cells = []) {
			state.cells = cells.map((cell,i)=> get_note_cell(cell, i, state))
			// state.track.getPattern().forEach((notes, pos) => {
...
export default ({ width, height }, screen) => {
